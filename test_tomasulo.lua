...
local yaml_fname = "./tomasulo/manual/commands.yaml"
local model_fname = "./tomasulo/manual/tomasulo.als"
...
local function run_sweep(model, run_command, sweeps, starting_point)
    ...
    f:write('"RegsAndEntries","Steps","Result","Time"\n')
...
for _, command in ipairs(table['commands']) do
    run_sweep("./tomasulo/manual/tomasulo.als", command, table['sweeps'], table['starting_point'])
end
