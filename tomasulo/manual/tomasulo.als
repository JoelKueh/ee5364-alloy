...
pred RobUpdateIssued[eIss: RobEntry] {
    ...
    all e: RobEntry | all r: eIss.Inputs' | (
        (e = eIss) implies (
            ...
            not (e in eIss.Deps') &&
            eIss.InputsProducedBy = eIss.InputsProducedBy + e.Instr
    ...
    // Handle dependence for issued instruction.
    //all e: RobEntry | (
    //    not (e.Output in eIss.Inputs') implies (
    //        // If output of previous is not input to current, then not dependence.
    //        not (e in eIss.Deps')
    //    ) else not (e.Output.Reserver = e) implies (
    //        // Non-transitive dependence graph.
    ...
    //        not (e in eIss.Deps')
    //    ) else (
    //        // In all other cases, we have a dependence.
    //        e in eIss.Deps'
    //    )
    //)
...
pred RobUpdatePassive[ePassive: RobEntry] {
    ...
    (not no WritebackEntry.ent && WritebackEntry.ent in ePassive.Deps) implies (
...
pred UpdateRob {
    ...
    not no IssueEntry.ent => RobUpdateIssued[IssueEntry.ent]
    not no WritebackEntry.ent => RobUpdateCompleted[WritebackEntry.ent]
    not no CommitEntry.ent => RobUpdateCommitted[CommitEntry.ent]
...
// Rules to update the ROB on each cycle.
fact {
    always {
        UpdateRegs
        UpdateRob
...
assert NoWARHazard_Complete {
	RobOrderedCommit => NoWAWHazard
}

assert NoWAWHazard_WAWVulnerable {
	RobRandomCommit => NoWAWHazard
}
...
run {RobOrderedCommit} for exactly 4 RobEntry, exactly 4 Register, 10 InstrRecord, 10 steps, 4 Int
